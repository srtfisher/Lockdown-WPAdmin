...
<form method="POST" action="<?php echo admin_url('admin.php?page=lockdown-private-users'); ?>">
  ...
  <p>
    Adding users below will <em>only</em> work if you have "Private Usernames/Passwords"
    selected for HTTP Authentication.
  </p>

  <div class="error">
    <p>
      <strong>Please note a few things:</strong>
      <ul>
        <li>
          1. If you are ever locked out, you can just delete the plugin files via
          FTP (<code>/wp-content/plugins/lockdown-wp-admin/</code>) and you will be able to login again.
        </li>
        <li>
          2. You cannot delete the current HTTP Authentication username you are using right now.
        </li>

        <li>
          3. Private user HTTP Authentication will not work if you don't have a username
          added below.
        </li>
      </ul>
    </p>
  ...
  <table class="widefat">
      ...
      <?php if ( isset( $private_users ) && count( $private_users ) > 0 ) : ?>
        ...
        <?php foreach ($private_users as $key => $user) : ?>
          <tr>
            ...
            <td>
              <a href="admin.php?page=<?php echo $_GET['page']; ?>&delete=<?php echo $key; ?>&_wpnonce=<?php echo $nonce; ?>">Delete</a>
            </td>
          </tr>
    ...
    <?php else : ?>
      <p>Private Username/Password HTTP Authentication <strong>is enabled</strong>.</p>
    <?php endif; ?>
